...
const UserDetailForm = ({ price, musicId, albumId, userId }) => {
  ...
  const onSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      ...
      const responseData = await fetchData({
        ...
        method: 'POST',
        data: {
          productId: musicId || albumId || userId,
          amount: price.toString(),
        },
      });
      if (buyerData.id && responseData) {
        if (albumId || musicId) {
          console.log(
            `/checkbuyertransaction?buyerId=${buyerData.id}&musicId=${albumId || musicId}&type=${albumId ? 'album' : 'music'}`
          );
          const transactionData = await fetchData({
            url: `/checkbuyertransaction?buyerId=${buyerData.id}&musicId=${albumId || musicId}&type=${
              albumId ? 'album' : 'music'
            }`,
            method: 'GET',
            data: {
          ...
          if (transactionData.exists) {
            setExistingError(`${albumId ? 'Album' : 'Music'} already purchased.`);
          } else {
            resetForm();
            navigate('/payment', {
              state: {
        ...
        <Form noValidate onSubmit={handleSubmit} style={{ maxWidth: '500px' }}>
          <Row className='mb-3'>
            ...
            <Form.Group as={Row} className='mt-3'>
              ...
              />
              <ErrorMessage name='mobile' component={Form.Control.Feedback} type='invalid' />
            </Form.Group>
          </Row>
          <div className='d-flex justify-content-center align-items-center'>
            <CustomButton
              type='submit'
              title={`${loading || isSubmitting ? 'Processing...' : 'Make a Payment'}`}
